...
| **Conditional / data-driven color** | Color by rules (if X then red) or by a *different* measure | PBI (fx: rules / gradient / field value — the reference), T (calc on Color / KPI shapes), SS (limited), Hex (limited) | [advanced] — PBI's fx conditional formatting is the benchmark |
...
- **From Tableau:** the Detail channel (LOD without color), reference *distributions* + box plots + scope (cell/pane/table), trend-model choice with confidence+stats, symmetric-log axis, custom-shape palettes, rich tooltip with inserted fields.
