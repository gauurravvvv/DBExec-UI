...
**Honest caveats on "have" claims:** the audit items marked *unknown* (÷0 guarding, total re-aggregation, dialect-gap behavior) are the ones most likely to be silently broken today given the ECharts-first, encoding-driven history. They are marked P0/P1 precisely because a confident-but-wrong number is worse than a missing feature — verify before trusting the existing aggregation path.