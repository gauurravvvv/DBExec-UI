...
3. **Label / Text channel + data-label show/hide** — labels must work on _every_ cartesian type, not a subset. (prior audit flagged "data labels everywhere")
...
9. **Constant + average reference line** — currently _missing entirely_; flagged in prior audit as a gap on all cartesian types. This is the single most visible P0 hole — every competitor has it, DBExec has none.
