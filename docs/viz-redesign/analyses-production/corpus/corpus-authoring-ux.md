...
**Biggest gap vs research:** DBExec has the _rendering_ layer (24 chart types, aggregation encoding, annotations) but is thin on the _authoring interaction_ layer — on-pill menus, drag polish, canvas manipulation (multi-select/align/z-order), undo/redo, and save-integrity guards. That interaction layer is exactly what separates "renders charts" from "lets people build analyses," and it is where the P0 effort concentrates.
