...
export function rateLimitMiddleware(bucket: keyof typeof buckets) {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      res.setHeader(
        'X-RateLimit-Reset',
        Math.floor(Date.now() / 1000 + result.msBeforeNext / 1000),
      );
      next();
    } catch (rej: any) {
      ...
      res
        .status(429)
        .json({ error: 'RATE_LIMITED', retryAfter: rej.msBeforeNext });
    }
  };
}
```
...
window.addEventListener('message', ev => {
  ...
  switch (msg.type) {
    case 'plugin.ready':
      postToPlugin({ type: 'data', payload: prepareDataForPlugin() });
      break;
    case 'plugin.dataRequest':
      postToPlugin({ type: 'data', payload: getMoreData(msg.range) });
      break;
    case 'plugin.event':
      handlePluginEvent(msg.event, msg.payload);
      break; // e.g. cell click
    case 'plugin.error':
      reportPluginError(msg.message, msg.stack);
      break;
  }
});
...
const createWebhookEndpoint = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await master_db_connection.close();
    sendResponse(res, true, CODE.SUCCESS, WH_MSG.CREATED, {
      ...ep,
