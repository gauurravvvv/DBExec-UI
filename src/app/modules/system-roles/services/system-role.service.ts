...
/**
 * SystemRoleService — list/view/CUD for system (master-DB) roles +
 * permission metadata.
 *
 ...
 * unchanged because the system-groups module + a couple other consumers
...
export class SystemRoleService {
  ...
  // Reads pipe through this Subject so callers (view-system-role /
  // edit-system-role ngOnDestroy) can cancel in-flight GETs when the user
  // navigates away — otherwise the XHR keeps running, the answer comes
  // back after a different component is mounted, and signals get clobbered
  ...
  async load(params: any) {
    ...
    try {
      const res: any = await lastValueFrom(
        this.http
          .apiGet(SYSTEM_ROLE.LIST, { params, skipLoader: true })
  ...
  async loadOne(roleId: string) {
    ...
    try {
      const res: any = await lastValueFrom(
        this.http
          .apiGet(SYSTEM_ROLE.GET + roleId, { skipLoader: true })
          .pipe(takeUntil(this._cancelReads$)),
      );
  ...
  async loadPermissions() {
    ...
    try {
      const res: any = await lastValueFrom(
        this.http
          .apiGet(SYSTEM_ROLE.LIST_PERMISSIONS, { skipLoader: true })
          .pipe(takeUntil(this._cancelReads$)),
      );
  ...
  //     so the grid can pre-select radios on Edit System Role.
  ...
  async add(data: AddRolePayload | any): Promise<any> {
    ...
    try {
      ...
      return await lastValueFrom(
        this.http.apiPost(SYSTEM_ROLE.ADD, payload, { skipLoader: true }),
      );
    } finally {
  ...
  async edit(
    ...
    try {
      ...
      return await lastValueFrom(
        this.http.apiPut(SYSTEM_ROLE.UPDATE + data.id, payload, {
          skipLoader: true,
        }),
      );
    } finally {
  ...
  async delete(id: string, justification?: string): Promise<any> {
    ...
    try {
      return await lastValueFrom(
        this.http.apiDelete(SYSTEM_ROLE.DELETE + id, {
          body: { justification },
          skipLoader: true,
        }),
      );
  ...
  // ── Legacy aliases kept for external callers (system-groups module) ────────
  ...
  viewRole(roleId: string) {
    return lastValueFrom(
      this.http.apiGet(SYSTEM_ROLE.GET + roleId, { skipLoader: true }),
    );
  }
  ...
  addRole(data: {
    ...
    return lastValueFrom(
      this.http.apiPost(SYSTEM_ROLE.ADD, data, { skipLoader: true }),
    );
  }
