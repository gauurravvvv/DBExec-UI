/**
 * Shared types for the relational RBAC system-role-management screens.
 *
 ...
 *
 * The write payload (POST /system-roles, PUT /system-roles/:id) is a
 * flat array of { permissionId, level } entries. Entries with level < 1
 * are stripped client-side before send — BE strips too, but keeping the
 * wire small helps when a role has hundreds of leaves and only a handful
 * granted.
 */
...
/** PUT /api/v1/system-roles/:id body. Wholesale-replaces the role's mappings. */
