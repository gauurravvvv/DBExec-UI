...
/**
 * Add System Role — permission-grid UX.
 *
 ...
 * selected access level (0..3). Defaults to READ on Add System Role per
...
@Component({
  ...
  styleUrls: ['./add-system-role.component.scss'],
  ...
  implements OnInit, OnDestroy, HasUnsavedChanges
{
  ...
  /**
   * Parallel-fetch the access-level table + the SYSTEM-scoped permission
  ...
  async loadMeta() {
    ...
    try {
      const [levels, modules] = await Promise.all([
        ...
        this.roleService.listPermissions({ scope: 'SYSTEM' }),
      ]);
