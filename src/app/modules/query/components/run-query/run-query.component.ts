  ...
  implements OnInit, OnDestroy, AfterViewInit, OnChanges
{
  ...
  private initMonaco(): void {
    ...
    setTimeout(() => {
      ...
      try {
        ...
        EditorCustomizationHelper.setupContentChangeListener(
          activeTab.editor,
          value => {
  ...
  executeQuery(): void {
    ...
    if (hasSelection) {
      ...
      const selectedText = activeTab.editor
        .getModel()
    ...
    const currentStatement = EditorCustomizationHelper.getCurrentStatement(
      activeTab.editor
    );
    if (currentStatement) {
  ...
  exportCurrentScript(): void {
    ...
    EditorCustomizationHelper.exportScript(activeTab);
  }
  ...
  onDatasetDialogClose(formData: DatasetFormData | null): void {
    ...
    if (formData) {
      ...
      // Get the SQL query
      const sql = activeTab.editor?.getValue() || activeTab.query;
  ...
  insertColumnName(
    ...
    const selection = activeTab.editor.getSelection();
    const text = DatabaseSchemaHelper.generateColumnReference(
      schemaName,
      tableName,
      columnName
    );
  ...
  getDatabaseTooltip(tab: QueryTab): string {
    ...
    const database = this.availableDatabases.find(
      db => db.id === tab.databaseId
    );
    if (!database) {
  ...
  closeOtherTabs(): void {
    ...
    const tabsToClose = TabManagementHelper.getOtherTabs(
      this.queryTabs,
      this.contextMenuTabId
    );
    ...
    const keepTab = this.queryTabs.find(
      tab => tab.id === this.contextMenuTabId
    );
    if (keepTab) {
  ...
  closeTabsToRight(): void {
    ...
    const tabIndex = this.queryTabs.findIndex(
      tab => tab.id === this.contextMenuTabId
    );
    if (tabIndex === -1 || tabIndex === this.queryTabs.length - 1) return;

    // Get tabs to the right
    const tabsToClose = TabManagementHelper.getTabsToRight(
      this.queryTabs,
      tabIndex
    );
    ...
    if (!this.queryTabs.find(tab => tab.id === this.activeTabId)) {
      const contextTab = this.queryTabs.find(
        tab => tab.id === this.contextMenuTabId
      );
      if (contextTab) {
  ...
  closeTabsToLeft(): void {
    ...
    const tabsToClose = TabManagementHelper.getTabsToLeft(
      this.queryTabs,
      tabIndex
    );
  ...
  private initMonacoForTab(tab: QueryTab): void {
    ...
    try {
      ...
      this.monacoIntelliSenseService.registerKeyboardShortcuts(tab.editor, () =>
        this.executeQueryForTab(tab)
      );
      ...
      console.error(
        `Error initializing Monaco Editor for tab ${tab.id}:`,
        error
      );
    }
  }
