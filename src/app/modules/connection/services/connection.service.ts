...
export class ConnectionService {
  ...
  async load(params: any) {
    ...
    try {
      const res: any = await lastValueFrom(this.http.apiGet(CONNECTIONS.LIST, { params }));
      if (res?.status) {
  ...
  async loadOne(orgId: string, id: string) {
    this._loading.set(true);
    try {
      const res: any = await lastValueFrom(this.http.apiGet(CONNECTIONS.VIEW + `${orgId}/${id}`));
  ...
  async add(form: FormGroup): Promise<any> {
    ...
    try {
      const { organisation, datasource, name, description, dbUsername, dbPassword } = form.value;
      return await lastValueFrom(this.http.apiPost(CONNECTIONS.ADD, {
  ...
  async update(form: FormGroup, justification?: string): Promise<any> {
    this._saving.set(true);
    try {
      const { id, name, description, organisation, datasource, status, dbUsername, dbPassword } = form.getRawValue();
      return await lastValueFrom(this.http.apiPut(CONNECTIONS.UPDATE, {
        id, name, description, organisation, datasource,
        status: status ? 1 : 0, dbUsername, dbPassword, justification,
      }));
    } finally { this._saving.set(false); }
  }

  async delete(orgId: string, id: string, justification?: string): Promise<any> {
    return await lastValueFrom(this.http.apiDelete(CONNECTIONS.DELETE + `${orgId}/${id}`, { body: { justification } }));
  }

  async bulkDelete(ids: string[], justification: string | undefined, orgId: string): Promise<any> {
    return await lastValueFrom(this.http.apiDelete(CONNECTIONS.BULK_DELETE + `${orgId}`, { body: { ids, justification } }));
  }
