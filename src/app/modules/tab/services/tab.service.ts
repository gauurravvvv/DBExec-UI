...
export class TabService {
  ...
  async load(params: any): Promise<void> {
    ...
    try {
      const res: any = await lastValueFrom(
        this.http.apiGet(TAB.LIST, { params }),
      );
      if (res?.status) {
  ...
  async delete(
    ...
    try {
      return await lastValueFrom(
        this.http.apiDelete(TAB.DELETE + `${orgId}/${tabId}`, {
          body: { justification },
        }),
      );
    } finally {
  ...
  deleteTab(orgId: string, id: string, justification?: string) {
    return lastValueFrom(
      this.http.apiDelete(TAB.DELETE + `${orgId}/${id}`, {
        body: { justification },
      }),
    );
  }
  ...
  addTab(tabForm: FormGroup) {
    ...
    return lastValueFrom(
      this.http.apiPost(TAB.ADD, { organisation, datasource, tabs }),
    );
  }
