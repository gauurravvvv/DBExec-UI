...
/**
 * TypedValueInput — one control that renders the right editor for a
 * predicate's right-hand value based on its `valueType`. This is the single
 * place the alerts surface maps a field's data type to an input control, so
 * the mapping never drifts across the condition rows:
 *
 ...
 *
 * Implemented as a ControlValueAccessor so it binds via `formControlName`
 * exactly like the other shared controls. For the `between`/`not_between`
 * operators the parent passes `range = true`, and the value is a two-element
 * tuple [lo, hi] rendered as two side-by-side typed inputs.
 */
@Component({
  selector: 'app-typed-value-input',
...
export class TypedValueInputComponent implements ControlValueAccessor {
  ...
  /** RHS value type — drives which editor is rendered. */
  @Input() valueType: AlertValueType = 'number';

  /** Two-input range mode (between / not_between). */
  @Input() range = false;

  /** Distinct string options (label/value) for the dropdown editor. */
  @Input() options: { label: string; value: string }[] = [];

  /** Whether the string dropdown should be editable (free-text) — true so a
   *  value not in the sampled distincts can still be typed. */
  ...
  /** Single-value model (number | string | boolean | Date-string). */
  value: any = null;
  /** Range tuple model. */
