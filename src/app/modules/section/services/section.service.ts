...
export class SectionService {
  ...
  async load(params: any): Promise<void> {
    ...
    try {
      const res: any = await lastValueFrom(this.http.apiGet(SECTION.LIST, { params }));
      if (res?.status) {
  ...
  async add(payload: any): Promise<any> {
    ...
    try { return await lastValueFrom(this.http.apiPost(SECTION.ADD, payload)); }
    finally { this._saving.set(false); }
  }

  async update(payload: any): Promise<any> {
    this._saving.set(true);
    try { return await lastValueFrom(this.http.apiPut(SECTION.UPDATE, payload)); }
    finally { this._saving.set(false); }
  }

  async delete(orgId: string, id: string, justification?: string): Promise<any> {
    this._saving.set(true);
    try { return await lastValueFrom(this.http.apiDelete(SECTION.DELETE + `${orgId}/${id}`, { body: { justification } })); }
    finally { this._saving.set(false); }
  }
