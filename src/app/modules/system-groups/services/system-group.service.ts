...
/**
 * SystemGroupService — list/view/CUD for master-DB (system) groups.
 *
 * Platform equivalent of GroupService, pointed at the /system-groups
 * endpoints. Group ↔ Role is many-to-many here: the payload carries a
 * `roleIds: string[]` (validated by the shared roleIdsSchema, min 1),
 * NOT a single `roleId`.
 *
 ...
 * delete button can spin independently. Every call passes
 * `{ skipLoader: true }` so the legacy global blocker stays out of this
 * module — the templates render skeleton placeholders while the signals
 * are true.
 */
...
export class SystemGroupService {
  ...
  async load(params: any) {
    ...
    try {
      const res: any = await lastValueFrom(
        this.http
          .apiGet(SYSTEM_GROUP.LIST, { params, skipLoader: true })
  ...
  async loadOne(groupId: string) {
    ...
    try {
      const res: any = await lastValueFrom(
        this.http
          .apiGet(SYSTEM_GROUP.GET + groupId, { skipLoader: true })
          .pipe(takeUntil(this._cancelReads$)),
      );
  ...
  async add(form: FormGroup): Promise<any> {
    ...
    try {
      // Group ↔ Role is many-to-many — send `roleIds` (array), not roleId.
   ...
   * control and send a precomputed full member list. The edit
   * component uses this to reassemble locked members (the logged-in
   * user) with the manageable picker selections — the BE requires the
   * COMPLETE membership set per save, and the picker only exposes the
   * manageable subset.
   */
  async edit(
    form: FormGroup,
  ...
  async delete(id: string, justification?: string): Promise<any> {
    ...
    try {
      return await lastValueFrom(
        this.http.apiDelete(SYSTEM_GROUP.DELETE + id, {
          body: { justification },
          skipLoader: true,
        }),
      );
  ...
  // Legacy-style convenience reads (pass skipLoader so they don't kick
  // the global blocker — callers drive their own loading state).
  ...
  viewGroup(groupId: string) {
    return lastValueFrom(
      this.http.apiGet(SYSTEM_GROUP.GET + groupId, { skipLoader: true }),
    );
  }
}
