...
export class GroupService {
  ...
  async load(params: any) {
    ...
    try {
      const res: any = await lastValueFrom(this.http.apiGet(GROUP.LIST, { params }));
      if (res?.status) {
  ...
  async loadOne(orgId: string, groupId: string) {
    this._loading.set(true);
    try {
      const res: any = await lastValueFrom(this.http.apiGet(GROUP.VIEW + `${orgId}/${groupId}`));
  ...
  async add(form: FormGroup): Promise<any> {
    ...
    try {
      ...
      return await lastValueFrom(this.http.apiPost(GROUP.ADD, { name, description, organisation, roleId, users }));
    } finally {
  ...
  async edit(form: FormGroup, justification?: string): Promise<any> {
    this._saving.set(true);
    try {
      const { id, name, description, status, users, organisation, roleId } = form.getRawValue();
      return await lastValueFrom(this.http.apiPut(GROUP.EDIT, {
        id, name, description, status: status ? 1 : 0, users, organisation, roleId, justification,
      }));
    } finally {
      this._saving.set(false);
    }
  ...
  async delete(orgId: string, id: string, justification?: string): Promise<any> {
    return await lastValueFrom(this.http.apiDelete(GROUP.DELETE + `${orgId}/${id}`, { body: { justification } }));
  }

  async bulkDelete(ids: string[], justification: string | undefined, orgId: string): Promise<any> {
    return await lastValueFrom(this.http.apiDelete(GROUP.BULK_DELETE + orgId, { body: { ids, justification } }));
  }
