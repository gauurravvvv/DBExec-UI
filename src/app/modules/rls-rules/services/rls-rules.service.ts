...
export class RlsRulesService {
  ...
  async load(orgId: string, datasourceId: string, params?: any) {
    ...
    try {
      const res: any = await lastValueFrom(
        this.http.apiGet(RLS_RULE.LIST, { params: { orgId, datasourceId, ...params } })
      );
      if (res?.status) {
  ...
  async loadOne(orgId: string, ruleId: string) {
    this._loading.set(true);
    try {
      const res: any = await lastValueFrom(this.http.apiGet(RLS_RULE.VIEW + `${orgId}/${ruleId}`));
  ...
  async loadAssignments(orgId: string, ruleId: string) {
    try {
      const res: any = await lastValueFrom(this.http.apiGet(RLS_RULE.LIST_ASSIGNMENTS + `${orgId}/${ruleId}`));
      if (res?.status) this._assignments.set(res.data?.assignments ?? res.data ?? []);
    } catch { this._assignments.set([]); }
  }

  async add(payload: any): Promise<any> {
    this._saving.set(true);
    try { return await lastValueFrom(this.http.apiPost(RLS_RULE.ADD, payload)); }
    finally { this._saving.set(false); }
  }

  async update(payload: any): Promise<any> {
    this._saving.set(true);
    try { return await lastValueFrom(this.http.apiPut(RLS_RULE.UPDATE, payload)); }
    finally { this._saving.set(false); }
  }

  async delete(orgId: string, ruleId: string, justification?: string): Promise<any> {
    this._saving.set(true);
    try { return await lastValueFrom(this.http.apiDelete(RLS_RULE.DELETE + `${orgId}/${ruleId}`, { body: { justification } })); }
    finally { this._saving.set(false); }
  }

  async addAssignment(payload: any): Promise<any> {
    this._saving.set(true);
    try { return await lastValueFrom(this.http.apiPost(RLS_RULE.ADD_ASSIGNMENT, payload)); }
    finally { this._saving.set(false); }
  }

  async deleteAssignment(orgId: string, assignmentId: string): Promise<any> {
    this._saving.set(true);
    try { return await lastValueFrom(this.http.apiDelete(RLS_RULE.DELETE_ASSIGNMENT + `${orgId}/${assignmentId}`, { body: {} })); }
    finally { this._saving.set(false); }
  }

  resetCurrent() { this._current.set(null); }
  resetAssignments() { this._assignments.set([]); }
