...
export class DashboardService {
  ...
  async load(params: any): Promise<void> {
    ...
    try {
      const res: any = await lastValueFrom(this.http.apiGet(DASHBOARD.LIST, { params }));
      if (res?.status) {
  ...
  async loadOne(orgId: string, id: string): Promise<void> {
    this._loading.set(true);
    try {
      const res: any = await lastValueFrom(this.http.apiGet(DASHBOARD.GET + `${orgId}/${id}`));
  ...
  async render(orgId: string, id: string): Promise<void> {
    this._loading.set(true);
    try {
      const res: any = await lastValueFrom(this.http.apiGet(DASHBOARD.RENDER + `${orgId}/${id}`));
  ...
  async delete(orgId: string, id: string, justification: string): Promise<any> {
    this._saving.set(true);
    try {
      return await lastValueFrom(this.http.apiDelete(DASHBOARD.DELETE + `${orgId}/${id}`, { body: { justification } }));
    } finally {
      this._saving.set(false);
    }
  }
  ...
  async bulkDelete(ids: string[], justification: string | undefined, orgId: string): Promise<any> {
    this._saving.set(true);
    try {
      return await lastValueFrom(this.http.apiDelete(DASHBOARD.BULK_DELETE + orgId, { body: { ids, justification } }));
    } finally {
      this._saving.set(false);
    }
  }
